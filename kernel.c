...
void process_setup(pid_t pid, const char* program_name) {
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        void *p = memory_virtual_to_physical(ptable[pid].pagetable ,(uintptr_t) seg.va());
        memset(p, 0, seg.size());
        memcpy(p, seg.data(), seg.data_size());
    }
    ...
    void *ptr = kalloc(stack_addr / PAGESIZE);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    void *p = memory_virtual_to_physical(current->pagetable, addr);
    memset(p, 0, PAGESIZE);
    return 0;
}
